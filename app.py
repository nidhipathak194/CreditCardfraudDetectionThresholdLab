...
<style>
    ...
    /* Active Selected Tab: Electric Blue Background & Bold White Text */
    ...
    [data-theme="dark"] .stTabs [data-baseweb="tab"] p {
        color: #FFFFFF !important;
        font-weight: 800 !important;
    }
...
with tab1:
    ...
    <div class="sorites-card">
        ...
        Consider a heap of sand: removing one grain at a time does not turn a heap into a non-heap (<b>Wright's Tolerance Principle</b>).
        However, repeated application of this premise leads to the absurd conclusion that a single grain of sand is still a heap.
        </p>
        <p>
        <b>The ML Analogy:</b> In credit card fraud detection, machine learning models assign a continuous risk probability that is the predicted fraud probability between 0 and 1.
        Assigning a rigid binary threshold creates an arbitrary operational split: two transactions possessing virtually identical risk profiles
        (difference in probability less than 0.001) receive opposite classifications (Block vs Approve).
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_def1:
        ...
        <div class="briefing-card">
            <h4>🎯 Rigid Decision Threshold (threshold)</h4>
            <p>
            A fixed numerical cutoff between 0 and 1 (e.g. threshold = 0.5000) used by binary classification systems.
            Transactions with estimated fraud risk at or above the threshold are classified as <b>Fraud (Block)</b>, whereas those below the threshold are classified as <b>Genuine (Approve)</b>.
    ...
    with col_def2:
        st.markdown(r"""
        <div class="briefing-card">
            <h4>📏 Tolerance Margin (epsilon (ε))</h4>
            <p>
            The infinitesimal risk divergence parameter (epsilon (ε) greater than 0) in Crispin Wright's (1975) Tolerance Principle.
            If two transactions A and B differ by less than twice epsilon (difference in probability = 2 × epsilon),
            they possess virtually identical risk profiles. Applying opposite operational decisions (Block vs Approve) solely due to an arbitrary cutoff (threshold) demonstrates boundary paradox in credit card scoring.
            </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown("Adjust the tolerance parameter epsilon (ε) to see how rigid decision thresholds force opposite business actions on two near-identical transactions:")
    ...
    st.info(f"💡 **Epsilon Paradox Note:** Total risk score divergence (difference in probability) is {2*epsilon_val:.4f}. Treating Transaction A as Fraud and Transaction B as Genuine is an operational artifact enforced by binary cutoffs, not a qualitative difference in transaction risk.")
    ...
    with st.expander("📚 Academic Underpinning & Citations (Wright, 1975; Eubulides)"):
        ...
        **Summary:** Classical first-order logic relies on crisp sets (in the set or not in the set). Real-world risk management requires partitioning a continuous space between 0 and 1 into discrete business actions. Threshold selection is inherently a value judgement balancing competing financial utilities.
        """)
...
with tab2:
    ...
    The background heatmap shows the model's predicted fraud probability between 0 and 1 across feature combinations:
    ...
    with st.expander("📚 Academic Reference: Class Imbalance & Fraud Benchmarking (Dal Pozzolo et al., 2015)"):
        ...
        **Key Takeaways:**
        - Real-world credit card datasets exhibit extreme class imbalance (roughly 0.172% positive rate).
...
with tab3:
    ...
    st.markdown("Deriving the optimal decision boundary τ* from economic business costs rather than default threshold = 0.50:")
    ...
    <div class="briefing-card">
        ...
        <ul>
            ...
            <li><b>Confusion Matrix Losses:</b> True Negatives (TN) cost $0; False Positives (FP) cost the number of FPs multiplied by the cost per FP; False Negatives (FN) cost the number of FNs multiplied by the cost per FN.</li>
            <li><b>Cost-Curve Minimization:</b> Plots total financial loss ($) across all thresholds between 0 and 1, demonstrating that default threshold = 0.50 causes severe financial loss compared to optimal τ*.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("Shows actual vs predicted transaction counts at Elkan's optimal threshold τ* = {:.4f}".format(tau_star))
    ...
    st.markdown(r"Demonstrates total monetary loss ($) across the entire threshold spectrum between 0 and 1. The golden star marks τ*:")
...
with tab4:
    ...
    <div class="briefing-card">
        ...
        <ul>
            <li><b>Probability Calibration:</b> Adjusting model output scores so that a predicted probability of 0.10 means exactly 10 out of 100 transactions are truly fraudulent (a predicted probability of p should match an actual fraud rate of about p).</li>
            <li><b>Platt Scaling (Sigmoid):</b> Fits a parametric logistic (S-shaped) curve to map raw model scores to calibrated probabilities.</li>
            ...
            <li><b>Brier Score Loss:</b> Mean squared error between predicted probabilities and actual outcomes that are 0 or 1 (average of (predicted probability − actual outcome)² across all transactions). Lower is better.</li>
            <li><b>Log-Loss (Cross-Entropy):</b> Evaluates probability logarithmic accuracy. Lower is better.</li>
            <li><b>Precision-Recall Area Under Curve (PR-AUC):</b> Evaluates classification performance under severe class imbalance (roughly 0.172%). Higher is better.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(r"Measures precision (Positive Predictive Value) vs recall (Sensitivity) across all decision thresholds under severe roughly 0.172% fraud imbalance:")
...
with tab5:
    ...
    <div class="briefing-card">
        ...
        <ul>
            ...
            <li><b>The Contested Zone [τ_min, τ_max]:</b> The Sorites indeterminacy interval — the predicted fraud probability falling between the lower and upper thresholds — where transaction labels flip depending on stakeholder cost preference.</li>
